...
class server_API {
    ...
    let Base_url = "https://api.darksky.net/forecast/e700e63b23dc86aa7f29a90be4c5fc2e/"
    
    let int_gone_msg = "You are disconnected from the internet.".capitalized
    
    func requestFor_NSMutableDictionary(Str_Request_Url:String , Request_parameter:[String: String]? , Request_parameter_Images:[String: UIImage]? ,status: @escaping (_ result: callingStatus) -> Void , response_Dictionary: @escaping (_ results: NSMutableDictionary) -> Void , response_Array: @escaping (_ results: NSMutableArray) -> Void){
        let myurl = NSURL(string: "\(Base_url)\(Str_Request_Url)")
        let obj_of_status = callingStatus(Status: false, Message: "Request Failed",Request_Url: "\(Base_url)\(Str_Request_Url)")
        print(myurl)
