...
class ServerAPI {
    ...
    static var tocken: String! = ""
    let apiBaseUrl = "http://192.168.200.70:3011/"
    ...
    func requestFor_NSMutableDictionary(Str_Request_Url:String, Str_Request_Method: String = "GET", Request_parameter:[String: Any]? , Request_parameter_Images:[String: UIImage]? ,isTokenEmbeded:Bool, status: @escaping (_ result: Calling_Status) -> Void , response_Dictionary: @escaping (_ results: NSMutableDictionary) -> Void, response_Array: @escaping (_ results: NSMutableArray) -> Void) {
        ...
        let myurl = NSURL(string: "\(apiBaseUrl)\(Str_Request_Url)")
        let obj_of_status = Calling_Status(Status: false, Message: "Request Failed",Request_Url: "\(apiBaseUrl)\(Str_Request_Url)")
    ...
    func requestFor_Upload_image_With_Data_And_Progress(delegate: URLSessionDelegate,Str_Request_Url:String , Request_parameter:[String: String]? , Request_parameter_Images:[String: UIImage]? , sessionDescription:String? , Compression_Ratio:Float?){
        ...
        DispatchQueue.global().async {
            let myurl = NSURL(string: "\(self.apiBaseUrl)\(Str_Request_Url)")
            let req = NSMutableURLRequest(url: myurl! as URL)
            if Request_parameter_Images != nil {
                req.httpMethod = "POST"
